...
for conc_Pb in conc_Pb_list:
	...
	CN = round(num_1_ave/N_Ge,3)
	NN2 = round(num_2_ave/N_Ge,3)
	NN3 = round(num_3_ave/N_Ge,3)
	NN4 = round(num_4_ave/N_Ge,3)
	NN5 = round(num_5_ave/N_Ge,3)
	...
	#print("Ge Ge Coordination number: ",CN)
	...
	os.chdir(anal_conc)
	with open ("MC_peak_information_GeGe.txt","w") as f1:
	...
	with open ("MC_peak_NN_vs_distance_GeGe.txt","w") as f1:
		for i in range(N_seg):
			f1.write(str(R_list_ave[i])+' '+str(M_ave[i])+'\n')
	f1.close()

	with open ("MC_RDF_NN_vs_distance_GeGe.txt","w") as f1:
		for i in range(N_seg):
	...
	#plt.title('Total number of Ge-Ge NN vs Distance')
	#plt.ylabel('Total GeGe NN')
