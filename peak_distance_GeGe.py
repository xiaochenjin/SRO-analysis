...
BC_CN_list=[[2.37,2.50],[2.36,2.51],[2.38,2.54]]
BC_NN2_list=[[3.72,4.49],[3.72,4.52],[3.70,4.54]]
BC_NN3_list=[[4.49,5.12],[4.52,5.17],[4.54,5.26]]
...
for conc_Pb in conc_Pb_list:
	...
	#with open ('./peak_information_GeGe.txt') as f2:
#		num1_info,num2_info,num3_info=np.loadtxt(f2,delimiter =' ',usecols=(0,1,2),unpack = True)

	#peak_dist=num2_info[2] #peak position of 2nd NN
	#width=num2_info[3] # half of peak width of 2nd NN
	#peak_dist=(num2_info[2]+num3_info[2])/2 #peak position of 2nd NN
	#width=num2_info[3]+0.5*num3_info[3] # half of peak width of 2nd NN
	#print(peak_dist)
	#print(width)
	...
	#		if abs(dist_list[l]-lc*neigh_dis[2])<0.5:
		...
		num_1=2*np.size(dist_1);num_2=2*np.size(dist_2);num_3=2*np.size(dist_3);num_4=2*np.size(dist_4);num_5=2*np.size(dist_5)
		num_1_list.append(num_1);num_2_list.append(num_2);num_3_list.append(num_3);num_4_list.append(num_4);num_5_list.append(num_5)
		num_bond=np.size(dist_1);num_bond_list.append(num_bond)
	...
	os.chdir(anal_conc)
	with open("data_neighbor_energy_GeGe.txt","w") as f1:
		cf = 0
		for i in index:
	...
	with open("data_neighbor_distance_GeGe.txt","w") as f1:
		cf = 0
		for i in index:
	...
	with open("neighbor_GeGe_distance.txt","w") as f1:
