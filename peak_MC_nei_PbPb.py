...
for conc_Pb in conc_Pb_list:
	...
	#	dr = 0.06
		...
		for i in range(N_Pb):
			...
			sx.append(s_vector[0])
			sy.append(s_vector[1])
			sz.append(s_vector[2])
		
		#Calculation of RDF
		dist_list=[];M=N_div*[0];GR_list=[]
		density_0=N_Pb/V 
		for i in range(N_Pb):
			for j in range(i+1,N_Pb):
				s_xij=sx[j]-sx[i]
				s_yij=sy[j]-sy[i]
				s_zij=sz[j]-sz[i]
	...
	CN = round(num_1_ave/N_Pb,3)
	NN2 = round(num_2_ave/N_Pb,3)
	NN3 = round(num_3_ave/N_Pb,3)
	NN4 = round(num_4_ave/N_Pb,3)
	NN5 = round(num_5_ave/N_Pb,3)
	...
	#print("Pb Pb Coordination number: ",CN)
	...
	num1_info = [CN,perf_peak_num1,dist_peak_num1,width_peak_num1]
	num2_info = [NN2,perf_peak_num2,dist_peak_num2,width_peak_num2]
	num3_info = [NN3,perf_peak_num3,dist_peak_num3,width_peak_num3]
	num4_info = [NN4,perf_peak_num4,dist_peak_num4,width_peak_num4]
	num5_info = [NN5,perf_peak_num5,dist_peak_num5,width_peak_num5]

	N_infor = len(num1_info)
	os.chdir(anal_conc)
	with open ("MC_peak_information_PbPb.txt","w") as f1:
	...
	with open ("MC_peak_NN_vs_distance_PbPb.txt","w") as f1:
		for i in range(N_seg):
			f1.write(str(R_list_ave[i])+' '+str(M_ave[i])+'\n')
	f1.close()

	with open ("MC_RDF_NN_vs_distance_PbPb.txt","w") as f1:
		for i in range(N_seg):
			f1.write(str(R_list_ave[i])+' '+str(GR_ave[i])+'\n')
	f1.close()

	#plt.xlabel('Distance (Angstrom) ')
	#plt.title('Total number of Pb-Pb NN vs Distance')
	#plt.ylabel('Total PbPb NN')
